...
def main_train():
    ...
    corpora_group.add_argument('--src2trg', metavar=('SRC', 'TRG'), nargs=2,
                               help='the source-to-target parallel corpus')
    corpora_group.add_argument('--trg2src', metavar=('TRG', 'SRC'), nargs=2,
    ...
    corpora_group.add_argument('--cache', type=int, default=1000000,
                               help='the cache size (in sentences) for corpus reading (defaults to 1000000)')
    corpora_group.add_argument('--cache_parallel', type=int, default=None,
    ...
    embedding_group.add_argument('--learn_encoder_embeddings', action='store_true',
                                 help='learn the encoder embeddings instead of using the pre-trained ones')
    embedding_group.add_argument('--fixed_decoder_embeddings', action='store_true',
                                 help='use fixed embeddings in the decoder instead of learning them from scratch')
    embedding_group.add_argument('--fixed_generator', action='store_true',
                                 help='use fixed embeddings in the output softmax instead of learning it from scratch')
    ...
    saving_group.add_argument('--save_interval', type=int, default=5000,
                              help='save intermediate models at this interval')
    saving_group.add_argument('--load_model',
                              help='prefix of file to load. e.g."test1.it30" if file names are test1.it30.src2src.dill')
    ...
    logging_group.add_argument('--validation_directions', nargs='+',
                               default=['src2src', 'trg2trg', 'src2trg', 'trg2src'], help='validation directions')
    logging_group.add_argument('--validation_output', metavar='PREFIX',
    ...
    if (args.src_embeddings is None or args.trg_embeddings is None) and (
            not args.learn_encoder_embeddings or args.fixed_decoder_embeddings or args.fixed_generator):
    ...
    def add_optimizer(module, directions=()):
        optimizer = tf.keras.optimizers.Adam(
            args.learning_rate)  # torch.optim.Adam(module.parameters(), lr=args.learning_rate)
    ...
    if args.src_embeddings is not None:
        ...
        src_embeddings.trainable = False
        if embedding_size == 0:
            embedding_size = src_embeddings.output_dim
        if embedding_size != src_embeddings.output_dim:
    ...
    if args.trg_embeddings is not None:
        ...
        trg_embeddings.trainable = False
        if embedding_size == 0:
            embedding_size = trg_embeddings.output_dim
        if embedding_size != trg_embeddings.output_dim:
            print('Embedding sizes do not match')
            sys.exit(-1)
    if args.learn_encoder_embeddings:
        print('removed the ability to learn the src encoder, talk to maxime if you want to fix that')
        sys.exit(-1)
        src_encoder_embeddings = data.random_embeddings(src_dictionary.size(), embedding_size)
        trg_encoder_embeddings = data.random_embeddings(trg_dictionary.size(), embedding_size)
    ...
    if args.fixed_decoder_embeddings:
        ...
        src_decoder_embeddings = data.random_embeddings(src_dictionary.size(), embedding_size)
        trg_decoder_embeddings = data.random_embeddings(trg_dictionary.size(), embedding_size)
    ...
    if args.fixed_generator:
        src_embedding_generator = EmbeddingGenerator(hidden_size=args.hidden, embedding_size=embedding_size)
        trg_embedding_generator = EmbeddingGenerator(hidden_size=args.hidden, embedding_size=embedding_size)
        ...
        src_generator = WrappedEmbeddingGenerator(src_embedding_generator, src_embeddings)
        trg_generator = WrappedEmbeddingGenerator(trg_embedding_generator, trg_embeddings)
    else:
        src_generator = LinearGenerator(args.hidden, src_dictionary.size())
        trg_generator = LinearGenerator(args.hidden, trg_dictionary.size())
    ...
    # Build encoder
    encoder = RNNEncoder(embedding_size=embedding_size, hidden_size=args.hidden,
                         bidirectional=not args.disable_bidirectional, nb_layers=args.layers, dropout=args.dropout)
    add_optimizer(encoder, (src2src_optimizers, trg2trg_optimizers, src2trg_optimizers, trg2src_optimizers))

    # Build decoders
    src_decoder = RNNAttentionDecoder(embedding_size=embedding_size, hidden_size=args.hidden, nb_layers=args.layers,
                                      dropout=args.dropout)
    trg_decoder = RNNAttentionDecoder(embedding_size=embedding_size, hidden_size=args.hidden, nb_layers=args.layers,
                                      dropout=args.dropout)
    ...
    src2src_translator = Translator(encoder_embeddings=src_encoder_embeddings,
                                    ...
                                    decoder=src_decoder, denoising=not args.disable_denoising)
    src2trg_translator = Translator(encoder_embeddings=src_encoder_embeddings,
                                    ...
                                    decoder=trg_decoder, denoising=not args.disable_denoising)
    ...
    if args.load_model is not None:
        # Load model
        with open('{0}.src2src.dill'.format(args.load_model), 'rb') as handle:
            src2src_translator = dill.load(handle)
        with open('{0}.src2trg.dill'.format(args.load_model), 'rb') as handle:
            src2trg_translator = dill.load(handle)
        with open('{0}.trg2trg.dill'.format(args.load_model), 'rb') as handle:
            trg2trg_translator = dill.load(handle)
        with open('{0}.trg2src.dill'.format(args.load_model), 'rb') as handle:
            trg2src_translator = dill.load(handle)

        # Load optimizer
        with open('{0}.src2src_optimizers.dill'.format(args.load_model), 'rb') as handle:
            src2src_optimizers = dill.load(handle)
        with open('{0}.trg2trg_optimizers.dill'.format(args.load_model), 'rb') as handle:
            trg2trg_optimizers = dill.load(handle)
        with open('{0}.src2trg_optimizers.dill'.format(args.load_model), 'rb') as handle:
            src2trg_optimizers = dill.load(handle)
        with open('{0}.trg2src_optimizers.dill'.format(args.load_model), 'rb') as handle:
            trg2src_optimizers = dill.load(handle)

        # src2src_trainer = Trainer(translator=src2src_translator, optimizers=src2src_optimizers, corpus=corpus,
        #                           batch_size=args.batch)
        # trainers[0] = src2src_trainer
        # loggers[2] = Logger('Source to source', src2src_trainer, src2src_validators, src2src_output, args.encoding)
    ...
    for i in range(0, len(args.validation), 2):
        ...
        if 'src2src' in args.validation_directions:
            src2src_validators.append(
                Validator(src2src_translator, src_validation, src_validation, args.batch, args.validation_beam_size))
        if 'trg2trg' in args.validation_directions:
            trg2trg_validators.append(
                Validator(trg2trg_translator, trg_validation, trg_validation, args.batch, args.validation_beam_size))
        if 'src2trg' in args.validation_directions:
            src2trg_validators.append(
                Validator(src2trg_translator, src_validation, trg_validation, args.batch, args.validation_beam_size))
        if 'trg2src' in args.validation_directions:
            trg2src_validators.append(
                Validator(trg2src_translator, trg_validation, src_validation, args.batch, args.validation_beam_size))

    # Create logging path
    if not os.path.exists("logs"):
        os.mkdir("logs")
    name = ["logs/src2src", "logs/trgback2src", "logs/trg2trg", "logs/srcback2trg", "logs/src2trg"]
    filename, extension = "logs/src2src", "_loss.txt"
    counter = 1
    ...
    for i in range(len(trainers)):
        trainers[i].name = name[i] + "_" + str(counter)
    log_extension = "_" + str(counter) + "_ppl.txt"
    ...
    def save_models(name):
        # Save models
        with open('{0}.{1}.src2src.dill'.format(args.save, name), 'wb') as handle:
            dill.dump(src2src_translator, handle)
        with open('{0}.{1}.trg2trg.dill'.format(args.save, name), 'wb') as handle:
            dill.dump(trg2trg_translator, handle)
        with open('{0}.{1}.src2trg.dill'.format(args.save, name), 'wb') as handle:
            dill.dump(src2trg_translator, handle)
        with open('{0}.{1}.trg2src.dill'.format(args.save, name), 'wb') as handle:
            dill.dump(trg2src_translator, handle)

        # Save optimizer
        with open('{0}.{1}.src2src_optimizers.dill'.format(args.save, name), 'wb') as handle:
            dill.dump(src2src_optimizers, handle)
        with open('{0}.{1}.trg2trg_optimizers.dill'.format(args.save, name), 'wb') as handle:
            dill.dump(trg2trg_optimizers, handle)
        with open('{0}.{1}.src2trg_optimizers.dill'.format(args.save, name), 'wb') as handle:
            dill.dump(src2trg_optimizers, handle)
        with open('{0}.{1}.trg2src_optimizers.dill'.format(args.save, name), 'wb') as handle:
            dill.dump(trg2src_optimizers, handle)

    # Training
    for step in range(1, args.iterations + 1):
        # count = 0
        # name = ["Source to source", "Target to source (backtranslation)",  "Target to target", "Source to target (backtranslation)", "Source to target"]
...
class Trainer:
    ...
    def step(self):
        ...
        self.src_word_count += sum(
            [len(data.tokenize(sentence)) + 1 for sentence in src])  # TODO Depends on special symbols EOS/SOS
        self.trg_word_count += sum(
            [len(data.tokenize(sentence)) + 1 for sentence in trg])  # TODO Depends on special symbols EOS/SOS
        ...
        with tf.GradientTape() as gt:
            ...
            self.loss += loss
        with open(self.name + '_loss.txt', 'a') as f:
            print(loss.numpy(), file=f)
        self.forward_time += time.time() - t

        # Backpropagate error + optimize
        t = time.time()
        gradients = gt.gradient(loss, self.translator.trainable_variables())  # loss.div(self.batch_size).backward()
        for optimizer in self.optimizers:
            optimizer.apply_gradients(zip(gradients, self.translator.trainable_variables()))
...
class Validator:
    def __init__(self, translator, source, reference, batch_size=50, beam_size=0):
        ...
        self.reference_word_count = sum([len(data.tokenize(sentence)) + 1 for sentence in
                                         self.reference])  # TODO Depends on special symbols EOS/SOS
        self.batch_size = batch_size
...
class Logger:
    def __init__(self, name, trainer, validators=(), output_prefix=None, encoding='utf-8', filename=None):
        self.name = name
            ...
            if step % self.log_interval == 0:
                print('  - Training:   {0:10.2f}   ({1:.2f}s: {2:.2f}tok/s src, {3:.2f}tok/s trg; epoch {4})'
                      .format(self.trainer.perplexity_per_word(), self.trainer.total_time(),
                              ...
                              self.trainer.corpus.epoch))
            with open(self.filename, 'a') as f:
                print(self.trainer.perplexity_per_word(), file=f)
