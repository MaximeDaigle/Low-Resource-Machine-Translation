...
def special_ids(ids):
    return ids * tf.dtypes.cast((ids < SPECIAL_SYMBOLS), tf.int64)


def word_ids(ids):
    # return (ids - SPECIAL_SYMBOLS + 1) * (ids >= SPECIAL_SYMBOLS).long()
    return (ids - SPECIAL_SYMBOLS + 1) * tf.dtypes.cast((ids >= SPECIAL_SYMBOLS), tf.int64)
...
def read_embeddings(file, threshold=0, vocabulary=None):
    ...
    embeddings = nn.Embedding(matrix.shape[0], dim, mask_zero=True, trainable=True)
    embeddings.build((None,)) # if you don't do this, the next step won't work
    set_embedding_layer_weights(embeddings, matrix)
